...
def generate_mnist_dataset(M_train, M_test, size=12, n_layers=8, n_qubits=6):
    ...
    # 对数据进行批量转换
    x_train = jnp.array([transform(img).numpy().reshape(size, size) for img in x_train]).reshape(-1, n_layers, n_qubits,
                                                                                                 3)
    x_test = jnp.array([transform(img).numpy().reshape(size, size) for img in x_test]).reshape(-1, n_layers, n_qubits,
                                                                                               3)
